...
async function render(p){try{if(p==='home')return renderHome();if(p==='explore')return renderExplore();if(p==='matches')return renderMatches();if(p==='groups')return renderGroups();if(p==='events')return renderEvents();if(p==='messages')return renderMessages();if(p==='calls')return renderCalls();if(p==='profile')return renderProfile();if(p==='compatibility')return renderCompatibility();if(p==='settings')return renderSettings();if(p==='vip')return renderVip();if(p==='admin')return renderAdmin()}catch(e){page.innerHTML=`<div class="pagepad"><div class="card"><h3>Something went wrong</h3><p class="muted">${esc(e.message)}</p><button class="primary" onclick="render('home')">Back to Discover</button></div></div>`}}
...
async function renderAdmin(){const [s,r,u,cr]=await Promise.all([api('/api/admin/stats'),api('/api/admin/reports'),api('/api/admin/users'),api('/api/admin/content-reports').catch(()=>[])]);page.innerHTML=`<div class="pagepad adminPage"><section class="sectionHero compactHero"><div><div class="eyebrow">🛡️ TRUST & SAFETY</div><h1>Moderation Center</h1><p>Review reports, restrict risky accounts and keep VibeMeet communities safe.</p></div></section><div class="grid"><div class="card"><h3>Users</h3><div class="score">${s.users}</div></div><div class="card"><h3>Online</h3><div class="score">${s.online}</div></div><div class="card"><h3>Open reports</h3><div class="score">${s.openReports}</div></div></div><div class="card" style="margin-top:18px"><h3>Profile reports</h3>${r.map(x=>`<div style="padding:14px;border-bottom:1px solid #eee"><b>${esc(x.reported_name)}</b> — ${esc(x.reason)}<div class="muted" style="margin:4px 0 9px">${esc(x.details||'No extra details')} · ${new Date(x.created_at).toLocaleString()}</div><div class="actions"><button class="secondary" onclick="moderateReport(${x.id},'dismiss')">Dismiss</button><button class="secondary" onclick="moderateReport(${x.id},'restrict')">Restrict</button><button class="danger" onclick="moderateReport(${x.id},'ban')">Ban</button><button class="secondary" onclick="moderateReport(${x.id},'resolve')">Resolve</button></div></div>`).join('')||'<p class="muted">No reports.</p>'}</div><div class="card" style="margin-top:18px"><h3>Community content reports</h3>${cr.map(x=>`<div style="padding:12px;border-bottom:1px solid #eee"><b>${esc(x.author_name||'Unknown author')}</b> · ${esc(x.reason)} <span class="muted">${new Date(x.created_at).toLocaleString()}</span><button class="secondary" onclick="resolveContentReport(${x.id})">Resolve</button></div>`).join('')||'<p class="muted">No content reports.</p>'}</div><div class="card" style="margin-top:18px"><h3>Recent users</h3>${u.slice(0,40).map(x=>`<div style="padding:10px;border-bottom:1px solid #eee;display:flex;justify-content:space-between;gap:10px"><span><b>${esc(x.name)}</b> · ${esc(x.status)}</span>${x.status!=='banned'?`<button class="danger" onclick="banUser(${x.id})">Ban</button>`:'<span class="muted">Banned</span>'}</div>`).join('')}</div></div>`}
